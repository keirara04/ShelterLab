...
export default function PostLabGigPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: '#000000' }}>
        <div className="glass-strong rounded-3xl p-10 text-center max-w-sm w-full">
          <h1 className="text-2xl font-black mb-3 text-white">Sign in to post</h1>
          <p className="text-gray-400 mb-6 text-sm leading-relaxed">Create an account to post a LabGig for your campus community.</p>
          <Link href="/login" className="text-white px-8 py-3 rounded-xl font-bold inline-block text-sm transition-opacity hover:opacity-90" style={{ background: 'linear-gradient(135deg, #14b8a6, #06b6d4)' }}>
            Go to Login
          </Link>
        </div>
      </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to post LabGig')
      }
      router.push('/labgigs')
    } catch (err) {
      setError(err.message || 'Failed to post LabGig')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <Link href="/labgigs" className="text-gray-500 hover:text-teal-400 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </Link>
            <h1 className="text-3xl md:text-4xl font-black text-white">Post a LabGig</h1>
          </div>
          <p className="text-gray-500 text-sm pl-8">Offer a service or post what you&apos;re looking for on campus</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="glass-strong rounded-3xl p-6 relative overflow-hidden">
            ...
            <div className="relative space-y-5">
              ...
              <div>
                ...
                />
                {profile?.kakao_link && (
                  <p className="text-[11px] text-teal-400/80 mt-1.5">Auto-filled from your profile — you can override it.</p>
                )}
              </div>
              ...
              >
                {loading ? 'Posting LabGig…' : 'Post LabGig'}
              </button>
            </div>
          </div>
          ...
          <div className="glass rounded-2xl p-4" style={{ borderColor: 'rgba(20,184,166,0.12)' }}>
            <p className="text-xs text-gray-500 leading-relaxed">
              <span className="text-teal-400 font-bold">Note:</span> LabGigs expire after 90 days. Mark as completed once the service is done!
            </p>
          </div>
        </form>
      </div>
